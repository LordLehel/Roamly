<!-- frontend/app/components/views/desktop/auth/VerifyEmailDesktop.vue -->
<template>
  <div :class="appConfig.layout.authWrapper">
    <UCard variant="glass">
      ...
      <div :class="appConfig.typography.authTitleWrapper">
        ...
        <p class="text-sm text-surface-400 text-center mt-1 max-w-xs mx-auto leading-relaxed">
          We sent a 6-digit code to
          <span class="font-semibold text-dark-text">{{ email }}</span
      ...
      <div :class="appConfig.layout.formWrapper">
        <div class="flex items-center justify-center gap-3 py-2">
          <input
            ...
            inputmode="numeric"
            maxlength="1"
            class="w-12 h-14 text-center text-2xl font-bold rounded-xl border-2 outline-none transition-all duration-150 bg-white/5 text-dark-text caret-brand-500 border-surface-500/30 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/30 disabled:opacity-40"
            :class="{
...
const onInput = (index: number, event: Event) => {
  ...
  // Accept only digits; strip extras that mobile autocomplete may inject
...
/* --- Clear cells and refocus when a verification error is received --- */
watch(
  () => props.error,
  (newErr) => {
    if (newErr) {
