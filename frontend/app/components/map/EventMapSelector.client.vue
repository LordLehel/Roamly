<template>
  <div class="relative w-full h-full min-h-[400px] flex flex-col">
    <div
      ref="mapContainer"
      class="w-full h-full z-0 rounded-md border-x border-t border-gray-200"
    ></div>

    <div
      class="w-full bg-surface-50 border border-gray-200 rounded-b-md p-4 flex flex-col sm:flex-row justify-between items-center gap-4 z-10 shrink-0"
    >
      ...
      <button
        ...
        :class="
          currentSelection ? 'bg-brand-500 hover:bg-brand-600' : 'bg-gray-400 cursor-not-allowed'
        "
...
const initmap = () => {
  ...
  const LControl = L.Control as unknown as {
    ...
    ) => L.Control & { on: (event: string, fn: (e: MarkGeocodeEvent) => void) => L.Control };
  };
