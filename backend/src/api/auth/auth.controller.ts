...
export const loginUser = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if(validLogin) {
            res.status(200).json({ status: 'success', message: 'Logged in succesfully!' });
        } else {
            res.status(500).json({ status: 'error', message: 'Incorrect email or password!' });
        }
    } catch (error) {
        console.error(error);
        res.status(500).json({ status: 'error', message: 'Error! There was an issue during the login process!' });
    }
}